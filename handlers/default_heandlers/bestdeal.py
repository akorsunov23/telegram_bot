...
from states.contact_information import StateBestdeal
...
def get_city(message: Message) -> None:
    """
    Хандлер, реагирующий на команду bestdeal.
    После инициализации команду начинает опрос пользователя и возвращает найденные отели по высокой цене.
    """
    bot.send_message(message.from_user.id, f'Вы выбрали поиск отелей по заданным параметрам \U00002705\n\n'
                                           f'{message.from_user.full_name} введите город для поиска \U0001F307')
    bot.set_state(message.from_user.id, StateBestdeal.city, message.chat.id)

    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        data['name'] = message.from_user.full_name


@bot.message_handler(state=StateBestdeal.city)
def get_region(message: Message) -> None:
    """ Хандлер, выдающий пользователю варианты на выбор региона. """
    if message.text:
        bot.set_state(message.from_user.id, StateBestdeal.region_selection, message.chat.id)
...
@bot.callback_query_handler(func=lambda call: True, state=StateBestdeal.region_selection)
def callback(call: CallbackQuery) -> None:
    ...
    """
    bot.set_state(call.from_user.id, StateBestdeal.amount_hostels, call.message.chat.id)
...
@bot.message_handler(state=StateBestdeal.amount_hostels)
def get_start_date(message: Message) -> None:
    ...
    if message.text.isdigit() and int(message.text) <= max_hotels:
        ...
        bot.set_state(message.from_user.id, StateBestdeal.start_date, message.chat.id)
...
@bot.message_handler(state=StateBestdeal.start_date)
    ...
    try:
        if bool(datetime.strptime(message.text, form_date)):
            if datetime.strptime(message.text, form_date) >= datetime.today():
                ...
                bot.set_state(message.from_user.id, StateBestdeal.end_date, message.chat.id)
...
@bot.message_handler(state=StateBestdeal.end_date)
def get_start_price(message: Message) -> None:
    """
    Хандлер, проверяет введенную дату на корректность, согласно шаблону,
    затем запрашивает желаемую максимальную цену за номер в отеле.
    """
    form_date = '%d-%m-%Y'
    try:
        if bool(datetime.strptime(message.text, form_date)):
            with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
                if datetime.strptime(message.text, form_date) > datetime.strptime(data['start_date'], form_date):
                    bot.send_message(message.from_user.id, 'Введите максимальную стоимость за номер в отеле \U000027A1',)
                    bot.set_state(message.from_user.id, StateBestdeal.start_price, message.chat.id)
...
@bot.message_handler(state=StateBestdeal.start_price)
def get_end_price(message: Message) -> None:
    """
    Хандлер, проверяет введенную стоимость на корректность,
    затем запрашивает желаемую минимальную цену за номер в отеле.
    """
    if message.text.isdigit():
        bot.send_message(message.from_user.id, 'Введите минимальную стоимость за номер в отеле \U000027A1')
        bot.set_state(message.from_user.id, StateBestdeal.end_price, message.chat.id)

        with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
            data['start_price'] = message.text
    else:
        bot.send_message(message.from_user.id, f'Цена может содержать только числа \U0001F937')


@bot.message_handler(state=StateBestdeal.end_price)
def get_start_distance(message: Message) -> None:
    """
    Хандлер, проверяет введенную стоимость на корректность,
    затем запрашивает желаемую максимальную удалённость отеля от центра.
    """
    if message.text.isdigit():
        bot.send_message(message.from_user.id, 'Введите максимальную удалённость отеля от центра \U000027A1')
        bot.set_state(message.from_user.id, StateBestdeal.start_distance, message.chat.id)
...
@bot.message_handler(state=StateBestdeal.start_distance)
def get_end_distance(message: Message) -> None:
    """
    Хандлер, проверяет введенную информацию на корректность,
    затем запрашивает желаемую минимальную удалённость отеля от центра.
    """
    if message.text.isdigit():
        bot.send_message(message.from_user.id, 'Введите минимальную удалённость отеля от центра \U000027A1')
        bot.set_state(message.from_user.id, StateBestdeal.end_distance, message.chat.id)

        with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
            data['start_distance'] = message.text
    else:
        bot.send_message(message.from_user.id, f'Дистанция может содержать только числа \U0001F937')


@bot.message_handler(state=StateBestdeal.end_distance)
def get_foto(message: Message) -> None:
    ...
    """
    if message.text.isdigit():
        bot.send_message(message.from_user.id, 'Хотели бы вы посмотреть фото? \U000027A1', reply_markup=request_foto())
        bot.set_state(message.from_user.id, StateBestdeal.foto, message.chat.id)
...
@bot.message_handler(state=StateBestdeal.foto)
def get_amount_foto(message: Message) -> None:
    ...
    if message.text.title() == "Да":
        ...
        bot.set_state(message.from_user.id, StateBestdeal.amount_foto, message.chat.id)
        ...
        with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
            ...
            text = '\U0000263A Спасибо за предоставленную информацию: ' \
                   ...
                   '\n\U00002714 Диапазон цен {start_pr}$ - {end_pr}$' \
                   '\n\U00002714 Удалённость от центра {start_ds} - {end_ds} ml' \
                   ...
                   '\n\nНачинаю поиск! \U000023F2 Пожалуйста, подождите'.format(
                    ...
                    start_pr=data['start_price'],
                    end_pr=data['end_price'],
                    start_ds=data['start_distance'],
                    end_ds=data['end_distance']
                    )
            bot.send_message(message.from_user.id, text)

            list_distance = [num for num in range(int(data['end_distance']), int(data['start_distance']))]

            info_for_user = search_hotel.search_info(id_region=data['id_region'], sort="PRICE_RELEVANT",
                                                     count=data['amount_hostels'], max_price=int(data['start_price']),
                                                     min_price=int(data['end_price']),
                                                     list_dist=list_distance,
                                                     start_data=data['start_date'], end_data=data['end_date'])
            for mg in info_for_user:
                db.add_data(id_user=message.from_user.id, command='/bestdeal',
                            date_time=str(datetime.today().strftime('%d-%m-%Y %H:%M')), info=mg)
                bot.send_message(message.from_user.id, mg)

            bot.set_state(message.from_user.id, StateBestdeal, message.chat.id)


@bot.message_handler(state=StateBestdeal.amount_foto)
    ...
    if message.text.isdigit() and int(message.text) <= max_foto:
        with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
            ...
            info_for_user = search_hotel.search_info(id_region=data['id_region'], count=data['amount_hostels'],
                                                     ...
                                                     max_price=int(data['start_price']),
                                                     min_price=int(data['end_price']), list_dist=list_distance,
                                                     start_data=data['start_date'], end_data=data['end_date'])
