...
from states.contact_information import StateLowprice
...
def get_city(message: Message) -> None:
    """
    Хандлер, реагирующий на команду lowprice.
    После инициализации команду начинает опрос пользователя и возвращает найденные отели по низкой цене.
    """
    bot.send_message(message.from_user.id, f'Вы выбрали поиск дешёвых отелей в городе \U00002705\n\n'
                                           f'{message.from_user.full_name} введите город для поиска \U0001F307')
    bot.set_state(message.from_user.id, StateLowprice.city, message.chat.id)

    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        data['name'] = message.from_user.full_name


@bot.message_handler(state=StateLowprice.city)
def get_region(message: Message) -> None:
    """ Хандлер, выдающий пользователю варианты на выбор региона. """
    if message.text:
        bot.set_state(message.from_user.id, StateLowprice.region_selection, message.chat.id)
...
@bot.callback_query_handler(func=lambda call: True, state=StateLowprice.region_selection)
def callback(call: CallbackQuery) -> None:
    ...
    """
    bot.set_state(call.from_user.id, StateLowprice.amount_hostels, call.message.chat.id)
...
@bot.message_handler(state=StateLowprice.amount_hostels)
def get_start_date(message: Message) -> None:
    ...
    if message.text.isdigit() and int(message.text) <= max_hotels:
        ...
        bot.set_state(message.from_user.id, StateLowprice.start_date, message.chat.id)
...
@bot.message_handler(state=StateLowprice.start_date)
    ...
    try:
        if bool(datetime.strptime(message.text, form_date)):
            if datetime.strptime(message.text, form_date) >= datetime.today():
                ...
                bot.set_state(message.from_user.id, StateLowprice.end_date, message.chat.id)
...
@bot.message_handler(state=StateLowprice.end_date)
        ...
        if bool(datetime.strptime(message.text, form_date)):
            with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
                if datetime.strptime(message.text, form_date) > datetime.strptime(data['start_date'], form_date):
                    ...
                    bot.set_state(message.from_user.id, StateLowprice.foto, message.chat.id)
...
@bot.message_handler(state=StateLowprice.foto)
def get_amount_photo(message: Message) -> None:
    ...
    if message.text.title() == "Да":
        ...
        bot.set_state(message.from_user.id, StateLowprice.amount_foto, message.chat.id)
        ...
        with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
            ...
            bot.set_state(message.from_user.id, StateLowprice, message.chat.id)


@bot.message_handler(state=StateLowprice.amount_foto)
def send_info_user(message: Message) -> None:
    ...
    if message.text.isdigit() and int(message.text) <= max_foto:
        with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
            ...
            info_for_user = search_hotel.search_info(id_region=data['id_region'], count=data['amount_hostels'],
                                                     sort="PRICE_LOW_TO_HIGH", start_data=data['start_date'],
                                                     end_data=data['end_date'], foto=data['foto'],
                                                     amount_foto=data['amount_foto'])
